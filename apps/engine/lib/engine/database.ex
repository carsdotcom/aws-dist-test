defmodule Engine.Database do
  ...
  def init([parent]) do
    ...
    {:ok, _} = Services.Registry.add(Services.Database, self())
  ...
  def handle_info({:DOWN, ref, _type, _pid, reason}, %{ref: ref} = state) do
    ...
    Logger.warn "Engine.Repo has crashed: #{inspect reason}"
  ...
  defp try_start() do
    case Engine.Repo.start_link([]) do
      {:ok, pid} ->
        pid
      {:error, {:already_started, _pid}} ->
        raise "Wasn't expecting Engine.Repo to be started but it was!"
      {:error, reason} ->
        Logger.warn "Unable to start Engine.Repo: #{inspect reason}"
        nil
    end
  end
end
